...
type OldServer interface {
	AddRoute(route rest.Route) error
	Start(ctx context.Context) error
}

type oldServerImpl struct {
	echo            *echo.Echo
	basePath        string
	port            uint16
...
func OldNewWithLogger(config Config, log logger.Logger) OldServer {
	echoServer := oldCreateEchoServer(logger.Wrap(log))
...
func (s *oldServerImpl) AddRoute(route rest.Route) error {
...
func (s *oldServerImpl) Start(ctx context.Context) error {
...
func (s *oldServerImpl) shutdown() error {
	ctx, cancel := context.WithTimeout(context.Background(), s.shutdownTimeout)
	defer cancel()
	return s.echo.Shutdown(ctx)
}

func oldCreateEchoServer(log echo.Logger) *echo.Echo {
	e := echo.New()
	e.HideBanner = true
	e.HidePort = true
	e.Logger = log

	oldRegisterMiddlewares(e)

	return e
}

func oldRegisterMiddlewares(e *echo.Echo) {
